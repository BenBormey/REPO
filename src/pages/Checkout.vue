    ...
    <form v-else-if="auth.isAuth" class="space-y-6" @submit.prevent="submit">
      ...
      <div class="card p-5">
        ...
        >
          ...
          <option v-for="s in services" :key="s.serviceId" :value="String(s.serviceId)">
            {{ s.name }} — ${{ money(s.price) }} / {{ s.durationMinutes }} min
          </option>
        </select>
        ...
        <div v-if="currentPreview" class="flex gap-4 items-center mt-4">
          ...
          <div>
            ...
            <div class="text-sm text-gray-600">
              ${{ money(currentPreview.price) }} · {{ currentPreview.durationMinutes }} min
            </div>
        ...
        <div v-if="fromCartMany" class="mt-3 space-y-2">
          <div v-for="it in cart.items" :key="it.id" class="flex items-center justify-between text-sm">
            <div class="flex items-center gap-3">
              <img :src="it.image" class="w-10 h-10 rounded object-cover" />
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        ...
        <label class="block">
          ...
          <input v-model="f.date" type="date" class="w-full border rounded-lg px-3 py-2" required />
        </label>
        <label class="block">
...
const currentPreview = computed(() => {
  if (fromCartOne.value) {
    ...
    return { name: it.name, price: it.price, durationMinutes: 0, description: '', imageUrl: it.image }
  }
